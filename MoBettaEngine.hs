...
printAction ic = do
  n <- ic --getting the result of the integer calculation (so x is an integer)
  doIO $ putStr (show n) --displays the computation after turning it into a string

-- Compute an integer, then store it
assignAction :: String -> IntCalc -> Action
assignAction v ic = do
  n <- ic --computes integer
  updateEnv v n --stores integer

-- Compute a boolean, use it to decide which computation to do.
ifAction :: BoolCalc -> Action -> Action -> Action
ifAction bc action1 action2 = do
  b <- bc --computes boolean
  if b then action1 else action2

whileAction :: BoolCalc -> Action -> Action
whileAction bc action = do
  b <- bc
  when b loop
  where
    loop = do
      action
      whileAction bc action
...
--fromMaybe e x means if x is Just n then the result is n otherwise the result is e
opLookup op opTable = fromMaybe (error "Undefined operator. Should never happen.") (lookup op opTable)
...
boolCalc (Reln op expr1 expr2) = do
  n1 <- (intCalc expr1)
  n2 <- (intCalc expr2)
  let comp = (opLookup op cOpTable)
  return $ comp n1 n2
boolCalc (BBin op expr1 expr2) = do
  n1 <- (boolCalc expr1)
  n2 <- (boolCalc expr2)
  let func = (opLookup op bBinOpTable)
  return $ func n1 n2
boolCalc (BUn op expr) = do
  n <- (boolCalc expr)
  let func = (opLookup op bUnOpTable)
  return $ func n
...
unOpTable = [(Neg, negate)]


intCalc :: AExpr -> IntCalc
intCalc (Var v) = retrieveEnv v --gets the value of the variable in the current environment
intCalc (IntConst val) = return val --this is the computation that returns simply the integer
--this computation needs to:
--1. calculate expr1
--2. calculate expr2
--3. return op expr1 expr2
intCalc (ABin op expr1 expr2) = do
  n1 <- (intCalc expr1)
  n2 <- (intCalc expr2)
  let func = (opLookup op binOpTable)
  return $ func n1 n2
